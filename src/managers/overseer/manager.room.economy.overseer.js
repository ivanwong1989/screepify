...
const runLinkTransfers = function(room) {
    ...
    for (const sender of senders) {
        ...
        if (result === OK) {
            debug(
                'overseer',
                `[Links] ${room.name} ${sender.id} -> ${receiver.id} ${amount}`
            );
        }
    }
};
...
var managerOverseer = {
    ...
    run: function(room) {
        ...
        // 2.5 Link transfers (source links -> controller/spawn links)
        runLinkTransfers(room);
