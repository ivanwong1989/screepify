...
async function uploadDev() {
    ...
    if (status.current !== 'dev') {
        log(`ℹ️  Current branch is '${status.current}'. Skipping local upload.`);
        log(`👉 Switch to 'dev' branch to sync with local server.`);
        return;
    }
    ...
    log(`🚀 Syncing 'dev' branch to Local Private Server...`);
